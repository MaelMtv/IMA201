...
alpha = 10
imf = filters.gaussian(im,1)

derx = dericheGradX(im, alpha)
dery = dericheGradY(im, alpha)

contours=np.uint8(maximaDirectionGradient(derx,dery))

plt.figure('Maxima du gradient Deriche dans la direction du gradient')
plt.imshow(255*contours)

dersx = dericheSmoothX(im, alpha)
dersy = dericheSmoothY(im, alpha)

contourss = np.uint8(maximaDirectionGradient(dersx, dersy))
plt.figure('Maxima du gradient Deriche smooth dans la direction du gradient')
plt.imshow(255*contourss)

io.imsave('dés_deriche.tif',255*contours)
